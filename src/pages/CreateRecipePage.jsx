...
import { parseIngredients, formatIngredient } from '../utils/ingredientParser'
import { 
  PhotoIcon, 
  XMarkIcon,
  PlusIcon,
...
export default function CreateRecipePage() {
  ...
  const [formData, setFormData] = useState({
    ...
    tags: [],
    source: '',
    family_id: '' // Add family_id to form state
  })
  ...
  const loadFamilies = async () => {
    try {
      ...
      // Check if user came from a family cookbook
      if (location.state?.from === 'family-cookbook' && location.state?.familyId) {
        setFormData(prev => ({
          ...prev,
          family_id: location.state.familyId
        }))
      }
    } catch (error) {
  ...
  const getBackNavigation = () => {
    ...
    if (location.state?.from === 'family-cookbook' && location.state?.familyId) {
      return `/family-cookbook/${location.state.familyId}`
    }
    // Check if user came from dashboard
    if (location.state?.from === 'dashboard') {
      return '/dashboard'
    }
    // Check if user came from a specific recipe
    if (location.state?.from === 'recipe-detail') {
      return location.state.recipeId ? `/cookbook/recipe/${location.state.recipeId}` : '/cookbook'
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const recipeData = {
        ...
        family_id: formData.family_id || null // Include family_id if selected
      }
      ...
      const newRecipe = await supabaseService.createRecipe(recipeData)
      
      // Navigate back to the appropriate page
      if (formData.family_id) {
        navigate(`/family-cookbook/${formData.family_id}`)
      } else {
        navigate(`/cookbook/recipe/${newRecipe.id}`)
      }
    } catch (error) {
      console.error('Error creating recipe:', error)
      console.error('Error details:', error.message)
      console.error('Error code:', error.code)
      console.error('Error hints:', error.hints)
      alert(`Error creating recipe: ${error.message || 'Unknown error'}. Please try again.`)
    } finally {
      setLoading(false)
    }
  }
                ...
                {families.length > 0 && (
                  <div>
                    ...
                    <select
                      ...
                      className="input"
                    >
                      <option value="">Keep Personal (don't share)</option>
                      {families.map((familyData) => (
                        <option key={familyData.family_id} value={familyData.family_id}>
                          {familyData.families.name}
                        </option>
                      ))}
                    </select>
                    <p className="mt-1 text-sm text-gray-500">
                      Share this recipe with your family members
                    </p>
                  </div>
                )}
              </div>
